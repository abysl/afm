...
| `./gradlew afmCiBuild -PafmRelease=true --no-configuration-cache` | Test delivery logic, Kotlin launcher/aliases, Rust, SDK, and AFM JVM; build unsigned APK/AAB, Linux `.deb`, and the Wasm site archive; archive reports |
...
not public assets. The Wasm production site is archived by `afmWebBundle` and stored
in the internal Forgejo bundle as `afm-<version>-web-wasm.tar.gz` for static-site
deployment; it is not a GitHub release asset. JS artifacts and browser acceptance are deferred: the
